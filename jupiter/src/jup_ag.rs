use {
    ...
    solana_sdk::{instruction::Instruction, pubkey::Pubkey, transaction::VersionedTransaction},
...
/// Hashmap, which key is the program id and value is the label. This is used to
/// help map error from transaction by identifying the fault program id. With
/// that, we can use the excludeDexes or dexes parameter.
...
pub async fn price(input_mint: Pubkey, output_mint: Pubkey, ui_amount: f64) -> Result<Price> {
    ...
    let input_data = response.data.get(&input_mint.to_string()).ok_or_else(|| {
        Error::JupiterApi(format!(
            "Input mint {input_mint} not found in response data"
        ))
    })?;
...
pub async fn quote(
    ...
    let url = format!(
        "{base_url}/quote?inputMint={input_mint}&outputMint={output_mint}&amount={amount}&\
         onlyDirectRoutes={}&{}{}{}{}{}{}{}{}{}{}{}",
        ...
        quote_config
            ...
            .map(|restrict_intermediate_tokens| format!(
        ...
        quote_config
            ...
            .map(|auto_slippage_collision_usd_value| format!(
...
/// Represents the priority levels for the `PriorityLevelWithMaxLamports`
/// variant.
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// used to help map error from transaction by identifying the fault program id.
/// With that, we can use the excludeDexes or dexes parameter.
